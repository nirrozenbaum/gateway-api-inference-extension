...
func (c *SchedulerConfig) WithPreSchedulePlugins(plugins ...framework.PreSchedule) *SchedulerConfig {
...
func (c *SchedulerConfig) WithFilters(filters ...framework.Filter) *SchedulerConfig {
	c.filters = filters
	return c
}
...
func (c *SchedulerConfig) WithScorers(scorers ...*framework.WeightedScorer) *SchedulerConfig {
	c.scorers = scorers
	return c
}
...
func (c *SchedulerConfig) WithPicker(picker framework.Picker) *SchedulerConfig {
	c.picker = picker
	return c
}
...
func (c *SchedulerConfig) WithPostSchedulePlugins(plugins ...framework.PostSchedule) *SchedulerConfig {
...
func (c *SchedulerConfig) WithPostResponsePlugins(plugins ...framework.PostResponse) *SchedulerConfig {
...
func (c *SchedulerConfig) AddPlugins(pluginObjects ...framework.Plugin) error {
	for _, plugin := range pluginObjects {
		if weightedScorer, ok := plugin.(*framework.WeightedScorer); ok {
			c.scorers = append(c.scorers, weightedScorer)
			plugin = weightedScorer.Scorer // if we got WeightedScorer, unwrap the plugin
		} else if scorer, ok := plugin.(framework.Scorer); ok { // if we got a Scorer instead of WeightedScorer that's an error.
